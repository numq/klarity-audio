#ifndef KLARITY_AUDIO_STRETCH_H
#define KLARITY_AUDIO_STRETCH_H
...
struct Stretch {
    ...
    Stretch() : randomEngine(std::random_device{}()) {}
    ...
    std::vector<float> process(Input *input, int inputSamples, float scaleFactor) {
        inputSamples /= channels;

        auto outputSamples = int(std::ceil((float) inputSamples / scaleFactor));
        ...
        for (int outputIndex = 0; outputIndex < outputSamples; ++outputIndex) {
                ...
                if (newSpectrum) {
                    for (int c = 0; c < channels; ++c) {
                        ...
                        for (int i = std::max<int>(0, -inputOffset); i < stft.windowSize(); ++i) {
                            timeBuffer[i] = input[(i + inputOffset) * channels + c];
                        }
                    ...
                    if (inputInterval != stft.interval()) {
                        ...
                        for (int c = 0; c < channels; ++c) {
                            ...
                            for (int i = std::max<int>(0, -prevIntervalOffset); i < stft.windowSize(); ++i) {
                                timeBuffer[i] = input[(i + prevIntervalOffset) * channels + c];
                            }
                            stft.analyse(c, timeBuffer);
                        }
                        for (int c = 0; c < channels; ++c) {
                            ...
                            auto &&spectrumBands = stft.spectrum[c];
                            for (int b = 0; b < bands; ++b) {
                                channelBands[b].prevInput = signalsmith::perf::mul(spectrumBands[b],
                                                                                   rotCentreSpectrum[b]);
                            }
                        }
                    }
            ...
            for (int c = 0; c < channels; ++c) {
                ...
                output[outputIndex * channels + c] = stftChannel[outputIndex];
            }
        }
...
private:
    ...
    static constexpr Sample maxCleanStretch{2};
    ...
    void processSpectrum(bool newSpectrum, Sample timeFactor) {
        ...
        std::uniform_real_distribution<Sample> timeFactorDist(
                maxCleanStretch * 2 * randomTimeFactor - timeFactor, timeFactor);
        ...
        for (int c = 0; c < channels; ++c) {
            ...
            for (int b = 0; b < bands; ++b) {
                ...
                prediction.energy *= std::max<Sample>(0,
                                                      mapPoint.freqGrad); // scale the energy according to local stretch factor
                ...
                if (b > 0) {
                    ...
                    if (b >= longVerticalStep) {
                        Complex longDownInput = getFractional<&Band::input>(c, mapPoint.inputBin -
                                                                               longVerticalStep *
                                                                               binTimeFactor);
                        prediction.longVerticalTwist = signalsmith::perf::mul<true>(prediction.input,
                                                                                    longDownInput);
                    } else {
        ...
        for (int b = 0; b < bands; ++b) {
            ...
            for (int c = 0; c < channels; ++c) {
                if (c != maxChannel) {
                    ...
                    Complex channelTwist = signalsmith::perf::mul<true>(channelPrediction.input,
                                                                        prediction.input);
